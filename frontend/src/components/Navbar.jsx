            ...
            <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="relative" ref={dropdownRef}>
                        ...
                        >
                            {!token ? (
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-300 group-hover:text-white">Account</span>
                            ) : (
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-300 group-hover:text-white hidden sm:inline">{user?.name?.split(' ')[0]}</span>
                            )}
                        ...
                        {isDropdownOpen && (
                            <div className="absolute right-0 mt-3 w-56 bg-black/80 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 ring-1 ring-white/5">
                                <div className="p-4 border-b border-white/5 bg-white/5">
                                    <p className="text-[10px] font-black text-purple-400 uppercase tracking-[0.2em] mb-1">Status</p>
                                    <p className="text-white font-bold text-sm tracking-tight">{token ? user?.name : 'Guest User'}</p>
                                    <p className="text-gray-500 text-[10px] font-medium">{token ? user?.email : 'Not signed in'}</p>
                                </div>
                                    ...
                                    {!token ? (
                                        <>
                                            <Link 
                                                ...
                                                className="flex items-center gap-3 w-full px-4 py-3 text-[11px] font-black uppercase tracking-widest text-gray-300 hover:text-white hover:bg-white/5 rounded-xl transition-all"
                                            >
                                            ...
                                            <Link 
                                                to="/register" 
                                                onClick={() => setIsDropdownOpen(false)}
                                                className="flex items-center gap-3 w-full px-4 py-3 text-[11px] font-black uppercase tracking-widest text-purple-400 hover:text-purple-300 hover:bg-purple-500/5 rounded-xl transition-all"
                                            >
                                        ...
                                        <>
                                            ...
                                            <button 
                                                ...
                                                className="flex items-center gap-3 w-full px-4 py-3 text-[11px] font-black uppercase tracking-widest text-red-500 hover:text-red-400 hover:bg-red-500/5 rounded-xl transition-all"
                                            >
